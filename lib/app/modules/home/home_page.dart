...
import 'package:marvel_api/app/components/widgets.dart';
...
class _HomePageState extends ModularState<HomePage, HomeController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        decoration: BoxDecoration(
            image:
                DecorationImage(image: AssetImage('lib/app/assets/wpp.jpg'), fit: BoxFit.cover )),
        child: Column(
          children: <Widget>[
            Center(
              child: Container(
  ...
  Widget results(List<Character> list) {
    if (list.isEmpty && controllerText.text != "") {
      return Center(child: Container(color: Colors.white70,child: Text('Nothing Found', style: TextStyle(color: Colors.red, fontSize: 20, fontFamily: 'RobotSlab'),)));
    }
    return Expanded(
      child: Padding(
                      ...
                      children: <Widget>[
                        ListTile(
                          trailing: IconButton(
                              ...
                              onPressed: () {
                                db.addFavorite(list[index]).then((_) {
                                  if (_ == 0) {
